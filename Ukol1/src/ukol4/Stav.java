...
//        this.poziceDiry = 5;
    ...
    public int hashCode() {
        ...
        hash = 97 * hash + this.data.hashCode();
        return hash;
    }
...
//            }
    ...
    Integer getVzdalenostH(Stav druhy) {
        //123456780
        //012 345 678
        //012 345 678
        int vzdalenost = 0;
        for (int i = 0; i < data.length(); i++) {
            int i2 = druhy.data.indexOf(data.charAt(i));
            if (i <= 2) {
                if (i2 <= 2) {
                    vzdalenost += Math.abs(i - i2);
                } else if (i2 <= 5) {
                    vzdalenost++;
                    vzdalenost += Math.abs(i + 3 - i2);
                } else {
                    vzdalenost += 2;
                    vzdalenost += Math.abs(i + 6 - i2);
                }
            } else if (i <= 5) {
                if (i2 <= 2) {
                    vzdalenost++;
                    vzdalenost += Math.abs(i - i2 - 3);
                } else if (i2 <= 5) {
                    vzdalenost += Math.abs(i - i2);
                } else {
                    vzdalenost++;
