 ...
 * @author olda9
 */
public class Stavy_listem_sirka4 {
    ...
    private List<Stav> sirka(Stav pocatek, Stav cil, HashMap<Stav, ArrayList<Stav>> graf) {
        Queue<Stav> zasobnik = new LinkedList<>();
        ArrayList<Stav> cesta = new ArrayList<>();
        zasobnik.add(pocatek);
        expandovano = 0;
        max = zasobnik.size() + cesta.size();
        while (!zasobnik.isEmpty()) {
            Stav aktualni = zasobnik.remove();
    ...
    private void naslednik(HashMap<Stav, ArrayList<Stav>> graf, Stav aktualni, Queue<Stav> zasobnik, List<Stav> cesta) {
        ArrayList<Stav> list;
        if ((list = graf.get(aktualni)) != null) {
            for (Stav naslednik : list) {
                if (!cesta.contains(naslednik) && !zasobnik.contains(naslednik)) {
                    zasobnik.add(naslednik);
                    expandovano++;
    ...
    public static void main(String[] args) {
        ...
        Stavy_listem_sirka4 s = new Stavy_listem_sirka4();
